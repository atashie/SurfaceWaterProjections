...
###############################################################
###############################################################
### Boxplots across XGBoost feature configurations
###############################################################
###############################################################
###############################################################
...
MODEL_ORDER = ["XGB - All Features", "XGB - Basic Features", "XGB - No Streamflow", 
               "XGB - Multi Objective", "XGB - 25% Noise"]

# Metrics to summarize
METRICS = ["MCC", "F1", "Accuracy", "Precision", "Recall", "ROC_AUC", "PR_AUC"]

# Output folder
OUTPUT_DIR = "xgboostFeatureComparisons"
...
def _highlight_boxes_vs_baseline(ax, df_plot: pd.DataFrame, metric: str, alpha: float = ALPHA):
    """
    For each model (except baseline "XGB - All Features"), run Mann–Whitney U tests 
    against the baseline on the given metric:
    - If significantly BETTER (alternative='greater', p < alpha): bold GOLD outline, blue fill
    - If significantly WORSE (alternative='less', p < alpha): bold RED outline, blue fill
    ...
    # Baseline (All Features) distribution
    baseline_vals = df_plot.loc[df_plot["model"] == "XGB - All Features", metric].dropna()
    ...
    # For each model, compute p-values vs baseline and apply highlighting
    for model in MODEL_ORDER:
        if model == "XGB - All Features":
            continue
        ...
        try:
            _, p_greater = mannwhitneyu(model_vals, baseline_vals, alternative="greater")
        except ValueError:
            p_greater = 1.0
        
        # Test if significantly WORSE (less)
        try:
            _, p_less = mannwhitneyu(model_vals, baseline_vals, alternative="less")
        except ValueError:
            p_less = 1.0

        # Apply styling based on results
        if p_greater < alpha:
            # Significantly better: bold gold outline, keep blue fill
            box.set_edgecolor("#FFD700")  # gold
            box.set_linewidth(3.0)
        elif p_less < alpha:
            # Significantly worse: bold red outline, keep blue fill
...
def plot_per_location_per_metric(
    ...
    metric values per model. y in [0, 1], x: 5 feature configurations.
    Gold outline = significantly better than baseline
    Red outline = significantly worse than baseline
    """
    ...
    for loc in locations:
        ...
        for metric in metrics:
            ...
            ax.set_title(f"{emsr_id} – per-date {metric}")
            ax.tick_params(axis='x', rotation=45)

            _highlight_boxes_vs_baseline(ax, df_plot, metric, alpha=ALPHA)

            plt.tight_layout()
            out_path = os.path.join(output_dir, f"{emsr_id}_{metric}_per_date_by_features.png")
...
def plot_per_metric_all_locations_panels(
    ...
    Each subplot: y in [0, 1], x: 5 feature configurations, boxplots for that metric in that location.
    Now also applies highlighting (gold=better, red=worse) vs baseline in each panel.
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    for metric in metrics:
        ...
        for idx, loc in enumerate(locations):
            ...
            if metric not in df_loc.columns:
                ...
                ax.set_title(f"{emsr_id} (no {metric})")
                ax.set_xticks([])
                ax.set_yticks([])
                continue
            ...
            if df_plot.empty:
                ax.set_title(f"{emsr_id} (no data)")
                ax.set_xticks([])
                ax.set_yticks([])
                continue
            ...
            ax.set_ylim(0.0, 1.0)
            ax.set_xlabel("")
            ax.set_ylabel(metric if idx % ncols == 0 else "")
            ax.set_title(emsr_id)
            ax.tick_params(axis="x", rotation=45)

            # Apply highlighting vs baseline within this panel
            _highlight_boxes_vs_baseline(ax, df_plot, metric, alpha=ALPHA)

        plt.suptitle(f"Per-date {metric} by feature configuration for each location", y=0.98)
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        out_path = os.path.join(
            output_dir,
            f"all_locations_{metric}_per_date_by_features.png",
        )
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()
...
def plot_aggregated_all_locations_and_save_stats(
    ...
    For each metric:
      ...
      - Produce a boxplot: y in [0, 1], x: 5 feature configurations.
      - Run Mann–Whitney U tests vs baseline across all locations/dates.
      - Highlight significantly better (gold outline) and worse (red outline) configurations.
    ...
    for metric in metrics:
        ...
        ax.set_xlabel("Feature Configuration")
        ax.set_ylabel(metric)
        ax.set_title(f"All locations – per-date {metric} by feature configuration")
        ax.tick_params(axis='x', rotation=45)
        ...
        out_path = os.path.join(
            output_dir,
            f"all_locations_aggregated_{metric}_per_date_by_features.png",
        )
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()

        # 2) Percentiles and p-values vs baseline
        baseline_vals = df_plot.loc[df_plot["model"] == "XGB - All Features", metric].dropna()
        ...
        for model in MODEL_ORDER:
            ...
            # Mann–Whitney U p-value vs baseline
            pcol = f"{metric}_pvalue"
            if model == "XGB - All Features" or baseline_vals.empty:
                pval = np.nan
            else:
                try:
                    _, pval = mannwhitneyu(model_vals, baseline_vals, alternative="greater")
                except ValueError:
                    pval = np.nan
...
def plot_all_metrics_summary(
    ...
    Bottom-right subplot is left blank.
    
    Each subplot shows aggregated boxplots (across all locations) for one metric,
    with gold/red outlining for significant differences vs baseline.
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Hide the 8th subplot (bottom right)
    axes[7].axis('off')
    
    # Add legend in the blank subplot
    ...
    axes[7].legend(handles=legend_elements, loc='center', fontsize=12, frameon=True, 
                   title='vs. XGB - All Features', title_fontsize=12)
    
    # Overall title
    fig.suptitle('XGBoost Feature Configuration Comparison\nAggregated Performance Across All Locations',
...
if __name__ == "__main__":
    ...
    # 2) Per-location, per-metric boxplots (with significance vs baseline)
