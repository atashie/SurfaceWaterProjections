"""
xgboostNoisyFloods.py - LABEL NOISE ROBUSTNESS TEST

XGBoost flood prediction with 25% symmetric label noise injected during training.
Tests model robustness to uncertain/noisy SAR-based flood masks.

Key differences from xgboostFloods.py:
- Injects 25% symmetric label noise during training ONLY
- Randomly flips 25% of water pixels → land and vice versa
- Maintains same class balance but introduces systematic label uncertainty
- Saves outputs to assessXGBoostNoisyModel/
- Labels outputs as "XGBoost-Noisy" for clarity

This tests H1: whether hydrologically-informed features enable robust learning
even when ground-truth labels contain substantial noise/uncertainty.
"""
...
class XGBoostFloodVisualizer(BaseVisualizer):
    def generate_all_outputs(self, output_folder: Optional[str] = None, **kwargs):
        if output_folder is None:
            output_folder = f"{self.training_folder}/assessXGBoostNoisyModel"
...
class XGBoostConfig:
    ...
    label_noise_fraction: float = 0.25  # 25% symmetric noise
...
def inject_symmetric_label_noise(y: np.ndarray, noise_fraction: float = 0.25, 
                                 seed: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
    """Inject symmetric label noise by swapping equal numbers of positive and negative labels.
    
    This simulates uncertainty in SAR-based flood masks where some water pixels may be
    misclassified as land and vice versa.
    
    Args:
        y: Original labels (0 or 1)
        noise_fraction: Fraction of positive labels to flip (default 0.25 = 25%)
        seed: Random seed for reproducibility
    
    Returns:
        Tuple of (noisy_labels, flip_mask) where flip_mask indicates which labels were flipped
    """
    if seed is not None:
    ...
    y_noisy = y.copy()
    flip_mask = np.zeros(len(y), dtype=bool)
    
    pos_idx = np.where(y == 1)[0]
    neg_idx = np.where(y == 0)[0]
    
    # Calculate number of labels to flip (symmetric)
    n_pos_flip = int(len(pos_idx) * noise_fraction)
    n_neg_flip = n_pos_flip  # Symmetric: flip same number of negatives
    
    # Ensure we don't try to flip more than available
    n_pos_flip = min(n_pos_flip, len(pos_idx))
    n_neg_flip = min(n_neg_flip, len(neg_idx))
    
    if n_pos_flip == 0 or n_neg_flip == 0:
        print(f"  Warning: Cannot inject noise (insufficient samples)")
        return y_noisy, flip_mask
    
    # Select random indices to flip
    pos_flip_idx = rng.choice(pos_idx, n_pos_flip, replace=False)
    neg_flip_idx = rng.choice(neg_idx, n_neg_flip, replace=False)
    
    # Flip labels
    y_noisy[pos_flip_idx] = 0  # Flip water → land
    y_noisy[neg_flip_idx] = 1  # Flip land → water
    
    # Mark flipped indices
    flip_mask[pos_flip_idx] = True
    flip_mask[neg_flip_idx] = True
    
    print(f"  ✅ Label noise injection: flipped {n_pos_flip} positives and {n_neg_flip} negatives")
    print(f"     Original class balance: {y.mean():.4f}")
    print(f"     Noisy class balance: {y_noisy.mean():.4f}")
    print(f"     Total flips: {flip_mask.sum()} ({flip_mask.mean():.2%})")
...
def load_data_streaming(parquet_dataset, watersheds: List[str],
    ...
    """Load data with LABEL NOISE INJECTION during training.
    
    Key modification: Injects 25% symmetric label noise ONLY during training mode.
    """
    ...
    for ws in watersheds:
        ...
        if mode == "train":
            ...
            # ✅ KEY MODIFICATION: Inject label noise ONLY during training
            y_noisy, flip_mask = inject_symmetric_label_noise(
                y_sampled, 
                noise_fraction=config.label_noise_fraction,
                seed=config.seed
            )
            
            X_ws = X_sampled
            y_ws = y_noisy  # Use noisy labels for training
    ...
    if mode == "train":
        print(f"    ⚠️  Training with NOISY labels: {len(y):,} pixels ({y.mean():.4f} positive rate)")
    else:
...
def train_xgboost_flood_model(parquet_uri: str, training_folder: str, 
    ...
    """Train XGBoost-Noisy model with label noise injection.
    ...
    # Construct output paths for NOISY model
    model_folder = f"{training_folder}/modelXGBoostNoisy"
    save_path = f"{model_folder}/xgb_noisy_model.pkl"
    
    print("=" * 70)
    print("XGBOOST-NOISY FLOOD MODEL")
    print(f"Training with {config.label_noise_fraction:.0%} SYMMETRIC LABEL NOISE")
    print("=" * 70)
    ...
    print(f"Label noise fraction: {config.label_noise_fraction:.1%} (applied to training only)")
    ...
    print(f"Features: {len(feature_cols)} (FULL FEATURE SET)")
    ...
    for fold_idx, fold in enumerate(folds):
        ...
        print(f"{'='*60}")
        print(f"  Train: {len(fold['train'])} watersheds (WITH NOISE)")
        print(f"  Val: {len(fold['val'])} watersheds (NO NOISE)")
        
        means, stds = compute_normalization_stats(dataset, feature_cols, fold['train'])
        
        # Load training data WITH noise injection
        ...
        # Load validation data WITHOUT noise
        ...
        print(f"  Training XGBoost-Noisy...")
        print(f"  Positive rates: train={y_train.mean():.4f} (NOISY), val={y_val.mean():.4f} (CLEAN)")
    ...
    print("\n" + "=" * 70)
    print("FINAL MODEL TRAINING (WITH NOISE)")
    print("=" * 70)
    ...
    if test_ws:
        ...
        if len(X_test) > 0:
            ...
            print(f"Test (CLEAN labels) MCC={test_metrics['mcc']:.4f}, F1={test_metrics['f1']:.4f}, "
    ...
    results = {
        ...
        'cv_results': folds_df,
        'label_noise_fraction': config.label_noise_fraction  # Track noise level
    }
    ...
    print("\n✅ XGBoost-Noisy training complete!")
    print(f"   Model trained with {config.label_noise_fraction:.0%} symmetric label noise")
    print(f"   Test performance on CLEAN labels evaluates robustness to noise")
...
def load_model_from_s3(model_path: str) -> Dict:
    """Load trained XGBoost-Noisy model from S3 pickle file."""
    import tempfile
    import pickle
    import boto3
    ...
    if model_path.startswith('s3://'):
        ...
        os.remove(temp_path)
        print(f"✅ XGBoost-Noisy model loaded from {model_path}")
        print(f"   Trained with {results.get('label_noise_fraction', 0.25):.0%} label noise")
    else:
