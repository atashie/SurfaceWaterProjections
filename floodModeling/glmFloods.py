"""
glmFloods.py - OPTIMIZED VERSION

Generalized Linear Model (GLM) baseline for flood prediction.
Uses logistic regression with proper sampling, hyperparameter tuning, and diagnostics.

Key improvements:
- Target-based sampling for cross-watershed balance
- Optional hyperparameter tuning (RandomizedSearchCV)
- Sampling-only class balancing
...
@dataclass
class GLMConfig:
    """Configuration for GLM flood model."""
    
    # GLM parameters (can be tuned)
    penalty: str = 'l1' #'l2'
    C: float = 0.006 #1.0
    solver: str = 'saga' #'lbfgs'
    max_iter: int = 2000 #1000
    l1_ratio: float = 0.5
    
    # Data sampling - ADAPTIVE TARGET-BASED
    use_adaptive_sampling: bool = True  # NEW: Enable adaptive sampling
    min_target_rate: float = 0.05  # Minimum target rate (very dry basins)
    max_target_rate: float = 0.50  # Maximum target rate (very wet basins)
...
def compute_watershed_prevalence(parquet_dataset, watersheds: List[str], 
    ...
    Returns:
    --------
    Dict mapping watershed_id -> positive_rate
    """
...
def compute_adaptive_target_rate_sqrt(natural_prevalence: float, 
    ...
    Square root compresses dynamic range while preserving relative differences:
    - Very dry (0.01) → ~0.05 (5x boost)
    - Moderate (0.25) → ~0.28 (1.1x boost)  
    - Wet (0.50) → ~0.50 (no change)
    ...
    """
    # Take square root to compress dynamic range
    sqrt_prev = np.sqrt(natural_prevalence)
    
    # Map from sqrt space to target range
    # sqrt(0.01) ≈ 0.1, sqrt(0.50) ≈ 0.71
    sqrt_min = np.sqrt(0.01)  # Lower bound of expected prevalence
    sqrt_max = np.sqrt(0.50)  # Upper bound of expected prevalence
...
def balanced_watershed_sample(X: np.ndarray, y: np.ndarray,
    ...
    target_pos_rate : float
        Desired positive rate (e.g., 0.25 = 25% positive)
    max_samples : int
        Maximum total samples
    min_samples_per_class : int
        Minimum samples per class (skip if below)
    
    Returns:
    --------
    Tuple of (X_sampled, y_sampled) or (None, None) if insufficient data
    """
    n = len(y)
    if n == 0:
        return None, None
    ...
    # Calculate samples needed for target rate
    # target_rate = n_pos / (n_pos + n_neg_sampled)
    # => n_neg_sampled = n_pos * (1 - target_rate) / target_rate
...
def load_data_streaming(parquet_dataset, watersheds: List[str],
                       ...
                       stds: np.ndarray, config: GLMConfig,
                       mode: str = "train",
    ...
    mode : str
        'train', 'val', or 'test'
    prevalence_map : Optional[Dict[str, float]]
        Natural prevalence per watershed (for adaptive sampling)
    """
    
    print(f"  Loading {mode} data from {len(watersheds)} watersheds...")
    
    # Create filter for all watersheds
    filt = ds.field("Watershed_Loc").isin(pa.array(watersheds))
    ...
    # Sample per watershed and accumulate
    X_parts = []
    y_parts = []
    skipped = []
...
def get_hyperparameter_grid():
    ...
    return {
        'C': loguniform(0.001, 100),  # Log-uniform for regularization
        'penalty': ['l1', 'l2'],
...
def spatial_kfold_split(watersheds: List[str], config: GLMConfig):
    """
    Create k-fold spatial splits for leave-one-watershed-out CV.
    
    Strategy:
    - Test watershed is always config.force_test_ws
    - Remaining watersheds undergo k-fold CV
    - Each fold: 1 watershed for validation, rest for training
    """
    
    # Seed watershed is always test
    test_ws = [config.force_test_ws]
    remaining = [w for w in watersheds if w != config.force_test_ws]
    
    # Shuffle remaining watersheds with fixed seed for reproducibility
    rng = np.random.RandomState(config.seed)
    shuffled = remaining[:]
    rng.shuffle(shuffled)
    
    # Create k folds: each watershed serves as validation once
    folds = []
    for fold_idx in range(config.n_folds):
        val_ws = [shuffled[fold_idx]]  # One watershed for validation
        train_ws = [w for w in shuffled if w != val_ws[0]]  # Rest for training
...
def train_glm_flood_model(parquet_uri: str, config: Optional[GLMConfig] = None,
                         save_path: Optional[str] = None) -> Dict:
    """Train GLM baseline model with k-fold CV and adaptive sampling."""
    
    config = config or GLMConfig()
    np.random.seed(config.seed)
    
    print("=" * 70)
    print("GLM BASELINE FLOOD MODEL - OPTIMIZED")
    print("=" * 70)
    ...
    # COMPUTE PREVALENCE FOR ADAPTIVE SAMPLING
    prevalence_map = None
    if config.use_adaptive_sampling:
    ...
    for fold_idx, fold in enumerate(folds):
        ...
        # Configure GLM
        if best_params:
            glm_params = best_params.copy()
            glm_params['random_state'] = config.seed + fold_idx
            glm_params['n_jobs'] = config.n_jobs
        else:
            glm_params = {
            ...
            if config.penalty == 'elasticnet':
                glm_params['l1_ratio'] = config.l1_ratio
                glm_params['solver'] = 'saga'
        
        # NO class_weight - using sampling only
        
        print(f"  Training GLM with params: {glm_params}")
        print(f"  Positive rates: train={y_train.mean():.4f}, val={y_val.mean():.4f}")
        
        glm = LogisticRegression(**glm_params)
        glm.fit(X_train, y_train)
        
        # Predictions
        y_train_proba = glm.predict_proba(X_train)[:, 1]
        y_val_proba = glm.predict_proba(X_val)[:, 1]
        ...
        # Cleanup
        del glm, X_train, y_train, X_val, y_val
        gc.collect()
    ...
    # Configure final GLM
    if best_params:
        final_glm_params = best_params.copy()
        final_glm_params['random_state'] = config.seed
        final_glm_params['n_jobs'] = config.n_jobs
    else:
        final_glm_params = {
            'penalty': config.penalty,
            'C': config.C,
        ...
        if config.penalty == 'elasticnet':
            final_glm_params['l1_ratio'] = config.l1_ratio
            final_glm_params['solver'] = 'saga'
    
    final_glm = LogisticRegression(**final_glm_params)
    final_glm.fit(X_train_final, y_train_final)
    ...
    if test_ws:
        ...
        if len(X_test) > 0:
            y_test_proba = final_glm.predict_proba(X_test)[:, 1]
    ...
    feature_importance_df = pd.DataFrame({
        ...
        'coefficient': final_glm.coef_[0],  # ADD THIS
        'abs_coefficient': np.abs(final_glm.coef_[0])
...
def create_diagnostic_plots(model_results: Dict, output_folder: str):
    ...
    # Helper function to save and upload
    def save_and_upload_plot(fig, s3_path):
        """Save plot to temp file and upload to S3."""
    ...
    # Get coefficient values (need to add to feature_importance in training)
    # For now, use importance as proxy
    colors = ['steelblue' for _ in range(len(top_features))]
    ...
    ax.set_title('GLM Feature Importance (Top 20)', fontsize=14, fontweight='bold')
    ax.grid(axis='x', alpha=0.3)
    
    # Add legend for positive/negative (if we have coefficient signs)
    if 'coefficient' in feature_importance.columns:
        colors = ['red' if top_features.iloc[i]['coefficient'] < 0 else 'blue' 
        ...
        red_patch = mpatches.Patch(color='red', label='Negative coefficient (↓ flood risk)')
        blue_patch = mpatches.Patch(color='blue', label='Positive coefficient (↑ flood risk)')
        ax.legend(handles=[red_patch, blue_patch])
    
    plt.tight_layout()
    importance_path = f"{output_folder}/glm_feature_importance.png"
    ...
    if 'coefficient' in feature_importance.columns:
        ...
        # Histogram
        axes[0].hist(feature_importance['coefficient'], bins=50, color='steelblue', alpha=0.7)
        axes[0].axvline(0, color='red', linestyle='--', linewidth=2)
        axes[0].set_xlabel('Coefficient Value', fontsize=12)
        axes[0].set_ylabel('Count', fontsize=12)
        axes[0].set_title('Distribution of GLM Coefficients', fontsize=13, fontweight='bold')
        axes[0].grid(alpha=0.3)
        
        # Sorted coefficients
        sorted_coefs = feature_importance['coefficient'].sort_values()
        axes[1].plot(range(len(sorted_coefs)), sorted_coefs, color='steelblue', linewidth=2)
        axes[1].axhline(0, color='red', linestyle='--', linewidth=2)
        axes[1].set_xlabel('Feature Rank', fontsize=12)
        axes[1].set_ylabel('Coefficient Value', fontsize=12)
        axes[1].set_title('Sorted Coefficient Values', fontsize=13, fontweight='bold')
        axes[1].grid(alpha=0.3)
        
        plt.tight_layout()
        coef_dist_path = f"{output_folder}/glm_coefficient_distribution.png"
        save_and_upload_plot(fig, coef_dist_path)
    ...
    if cv_results is not None and not cv_results.empty:
        ...
        plt.tight_layout()
        cv_path = f"{output_folder}/glm_cv_performance.png"
...
class GLMFloodVisualizer:
    ...
    def generate_all_outputs(self, output_folder: Optional[str] = None):
        ...
        if output_folder is None:
            output_folder = f"{self.training_folder}/assessGLMModel"
        
        print("\n" + "="*60)
        print("GENERATING GLM MODEL VISUALIZATION OUTPUTS")
        print("="*60)
        ...
        # Create outputs
        pred_tiff = f"{output_folder}/glm_predictions_{self.test_watershed}.tif"
        cm_tiff = f"{output_folder}/glm_confusion_matrix_{self.test_watershed}.tif"
        
        self._write_multiband_tiff(ref_ds, predictions, dates, pred_tiff, "GLM_Prediction")
        self._write_multiband_tiff(ref_ds, confusions, dates, cm_tiff, "GLM_CM")
    ...
    def _create_pngs(self, confusions, dates, metrics, output_folder):
        ...
        for date_str in dates_to_process:
            ...
            ax.set_title(f'GLM Baseline Confusion Matrix - {date_str}', fontsize=14, fontweight='bold')
            ...
            s3_path = f"{output_folder}/glm_confusion_matrix_{date_str}.png"
            if s3_path.startswith('s3://'):
                bucket, key = s3_path.replace('s3://', '').split('/', 1)
    ...
    def _write_csvs(self, output_folder, per_date_metrics):
        ...
        if per_date_metrics:
            ...
            s3_path = f"{output_folder}/glm_per_date_metrics_{self.test_watershed}.csv"
            if s3_path.startswith('s3://'):
                bucket, key = s3_path.replace('s3://', '').split('/', 1)
        ...
        if metrics_summary:
            ...
            if rows:
                ...
                s3_path = f"{output_folder}/glm_metrics_summary.csv"
                if s3_path.startswith('s3://'):
                    bucket, key = s3_path.replace('s3://', '').split('/', 1)
                    self.s3_client.upload_file(tmp_csv, bucket, key)
                os.remove(tmp_csv)
        ...
        if feature_importance is not None:
            ...
            s3_path = f"{output_folder}/glm_feature_importances.csv"
            if s3_path.startswith('s3://'):
                bucket, key = s3_path.replace('s3://', '').split('/', 1)
                self.s3_client.upload_file(tmp_csv, bucket, key)
            os.remove(tmp_csv)
