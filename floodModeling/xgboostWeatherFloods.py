"""
xgboostWeatherFloods.py - WEATHER-ONLY FEATURES

XGBoost flood prediction using ALL static geospatial data and weather time series,
but NO streamflow variables. This tests whether streamflow predictions add value
beyond weather data alone.

This serves to test H1: whether explicit streamflow modeling improves performance
compared to using weather forcings alone.

Key differences from xgboostFloods.py:
- Uses all static features (DEM, HAND, soils, land cover)
- Uses all weather/meteorological time series
- EXCLUDES any variable containing "streamflow"
- Saves outputs to assessXGBoostWeatherModel/
- Labels outputs as "XGBoost-Weather" for clarity
"""
...
class XGBoostFloodVisualizer(BaseVisualizer):
    def generate_all_outputs(self, output_folder: Optional[str] = None, **kwargs):
        if output_folder is None:
            output_folder = f"{self.training_folder}/assessXGBoostWeatherModel"
...
def get_weather_only_feature_columns(dataset) -> List[str]:
    """Get all feature columns EXCEPT streamflow variables.
    
    Weather-only model uses:
    - All static geospatial features (DEM, HAND, soils, land cover)
    - All weather/meteorological time series (precipitation, temperature, etc.)
    - NO streamflow variables (local or basin-scale)
    
    This tests whether explicit streamflow modeling adds value beyond weather alone.
    
    Returns:
        List of feature column names excluding any containing "streamflow"
    """
    ...
    for field in dataset.schema:
        ...
        # Check if column contains "streamflow" (case-insensitive)
        if "streamflow" in field.name.lower():
            excluded_streamflow.append(field.name)
            continue
        
        # Include all numeric features that are NOT streamflow
        t = field.type
        if pa.types.is_floating(t) or pa.types.is_integer(t):
            feature_cols.append(field.name)
    
    print(f"  ✅ Weather-only features selected: {len(feature_cols)} features")
    print(f"     Excluded {len(excluded_streamflow)} streamflow variables:")
    for sf in excluded_streamflow:
        print(f"       - {sf}")
    print(f"     This tests whether streamflow adds value beyond weather alone")
...
def train_xgboost_flood_model(parquet_uri: str, training_folder: str, 
    ...
    """Train XGBoost-Weather model with k-fold CV and adaptive sampling.
    ...
    # Construct output paths for WEATHER model
    model_folder = f"{training_folder}/modelXGBoostWeather"
    save_path = f"{model_folder}/xgb_weather_model.pkl"
    
    print("=" * 70)
    print("XGBOOST-WEATHER FLOOD MODEL")
    print("Using all geospatial + weather features, NO streamflow")
    print("=" * 70)
    ...
    # ✅ KEY CHANGE: Use weather-only features (exclude streamflow)
    feature_cols = get_weather_only_feature_columns(dataset)
    
    print(f"Watersheds: {len(watersheds)}")
    print(f"Features: {len(feature_cols)} (WEATHER-ONLY, NO STREAMFLOW)")
    ...
    for fold_idx, fold in enumerate(folds):
        ...
        print(f"  Training XGBoost-Weather...")
...
#        final_xgb_params['early_stopping_rounds'] = config.early_stopping_rounds
    ...
    print("\n✅ XGBoost-Weather training complete!")
...
def load_model_from_s3(model_path: str) -> Dict:
    """Load trained XGBoost-Weather model from S3 pickle file."""
    import tempfile
    import pickle
    import boto3
    ...
    if model_path.startswith('s3://'):
        ...
        os.remove(temp_path)
        print(f"✅ XGBoost-Weather model loaded from {model_path}")
    else:
