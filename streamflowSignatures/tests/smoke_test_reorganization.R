...
# Run from project root: Rscript tests/smoke_test_reorganization.R
################################################################################
...
test_files <- c(
  "tests/smoke_test.R",
  "tests/test_climate_functions.R",
  "tests/test_climate_signatures.R",
  "tests/qa_qc_signatures.R"
)
...
run_test("orphans/ directory exists", quote({
  if (!dir.exists(file.path(main_dir, "orphans"))) stop("Directory not found")
}))
