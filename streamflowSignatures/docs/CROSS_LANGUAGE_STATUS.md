...
All three runners compute the same 12 `flagged_*` QA/QC columns on the assembled output table. **They were never value-compared by the acceptance gates** (which score column sets and numeric identity-R²), and on 2026-09-04 that gap was found to hide a real divergence: 11 flags agreed cell-for-cell on the WY 1993–2025 reference set, but `flagged_for_high_na` was computed over a different denominator in each language (Julia: 16 numeric metadata columns only, 1,224 flagged; Python: every non-flag column, 787; rpkg: emitted `_mean`/`_median` keys, 90). Fixed the same day with one shared definition (config `qa_qc.high_na_denominator`: the signature columns present in the table, NA/NaN fraction > 0.30) — Julia and Python now agree on all 6,678 gages (791); rpkg's remaining 20 differed because its 2026-08-27 CSV carries tau = 1 / p = 1 for constant series where canonical emits NaN — fixed in rpkg the same day (`mann_kendall_test()` wrapper with the canonical NA contract, used by `generate_stats()` and the Pettitt segment p-values); the rpkg reference CSV needs a benchmark re-run to reflect it. Future acceptance runs should compare the 12 boolean columns exactly. Monolithic R computes equivalent flags via `qa_qc_signatures.R` as a separate post-processing step (with STRICT ordering in the order checks, unlike the three packages).
