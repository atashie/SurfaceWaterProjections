...
| 2 | **Canadian daily streamflow** (m³/s) + station regulation metadata (RHBN, REGULATED via `hy_stn_regulation`) | Same signature pipeline; regulation flags feed `human_interference_class` (exported to `metadata/canadian_hydat_interference.csv`) | Water Survey of Canada / Environment and Climate Change Canada (HYDAT database) | https://collaboration.cmc.ec.gc.ca/cmc/hydrometrics/www/ (HYDAT SQLite archive; R package `tidyhydat`) |
...
| 4 | **Daymet daily climate** — PPT, tmin/tmax, SWE, vapor pressure, solar radiation, 1980–2023, 1 km | Climate-dependent signatures: runoff ratios, elasticity, Q-P seasonality, storage (`daymet_1980_2023.parquet`, joined at runtime) | ORNL DAAC (NASA) | https://daymet.ornl.gov (Daymet V4, ORNL DAAC) |
