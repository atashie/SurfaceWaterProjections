"""
Compare Python benchmark output against Golden Julia (canonical) reference.

Produces:
  - Console summary
  - docs/benchmarks/python_vs_golden_julia_comparison.csv   (per-column detail)
  - docs/benchmarks/python_vs_golden_julia_summary.md        (high-level report)
...
PYTHON_PATH = SCRIPT_DIR / "python_signatures.csv"

OUTPUT_CSV = SCRIPT_DIR / "python_vs_golden_julia_comparison.csv"
OUTPUT_MD = SCRIPT_DIR / "python_vs_golden_julia_summary.md"
...
def main():
    print("=" * 80)
    print("PYTHON vs GOLDEN JULIA (CANONICAL) REFERENCE")
    print("=" * 80)
    ...
    if not PYTHON_PATH.exists():
        print(f"\nERROR: Python benchmark output not found: {PYTHON_PATH}")
        print("Run `python docs/benchmarks/run_python_benchmark.py` first")
        return 1
    ...
    print("Loading Python benchmark output...")
    py_df = pd.read_csv(PYTHON_PATH, low_memory=False)
    py_df["gage_id"] = py_df["gage_id"].astype(str).str.strip()
    print(f"  {py_df.shape[0]:,} gages x {py_df.shape[1]:,} cols")

    # Common gages
    jl_gages = set(jl_df["gage_id"])
    py_gages = set(py_df["gage_id"])
    common_gages = sorted(jl_gages & py_gages)
    print(f"\n  Common gages: {len(common_gages):,}")
    print(f"  Only in Golden Julia: {len(jl_gages - py_gages):,}")
    print(f"  Only in Python: {len(py_gages - jl_gages):,}")

    jl = jl_df[jl_df["gage_id"].isin(common_gages)].set_index("gage_id").sort_index()
    py = py_df[py_df["gage_id"].isin(common_gages)].set_index("gage_id").sort_index()

    # Column mapping
    jl_sig_cols = {c for c in jl.columns if is_signature_col(c)}
    py_sig_cols = {c for c in py.columns if is_signature_col(c)}

    jl_norm = {normalize_col(c): c for c in jl_sig_cols}
    py_norm = {normalize_col(c): c for c in py_sig_cols}

    common_norms = sorted(set(jl_norm) & set(py_norm))
    jl_only_norms = sorted(set(jl_norm) - set(py_norm))
    py_only_norms = sorted(set(py_norm) - set(jl_norm))

    print(f"\n  Common signature columns: {len(common_norms):,}")
    print(f"  Only in Golden Julia: {len(jl_only_norms)}")
    print(f"  Only in Python: {len(py_only_norms)}")
    ...
    for norm_col in common_norms:
        ...
        py_vals = pd.to_numeric(py[py_col], errors="coerce").values

        r2, n_valid = r2_identity(jl_vals, py_vals)
        rho, _ = spearman_corr(jl_vals, py_vals)
        na_mm = count_na_mismatch(jl_vals, py_vals)

        jl_na = int((~np.isfinite(jl_vals)).sum())
        py_na = int((~np.isfinite(py_vals)).sum())

        mask = np.isfinite(jl_vals) & np.isfinite(py_vals)
        if mask.sum() > 0:
            diff = py_vals[mask] - jl_vals[mask]
    ...
    generate_summary_report(res, common_norms, jl_only_norms, py_only_norms,
                            len(common_gages), jl_df.shape[0], py_df.shape[0],
                            jl_norm, py_norm)
...
def print_console_summary(res):
    ...
    print(f"\n{'=' * 80}")
    print("OVERALL ALIGNMENT: Python vs Golden Julia (Canonical)")
    print(f"{'=' * 80}")
...
def generate_summary_report(res, common_norms, jl_only_norms, py_only_norms,
                            n_common_gages, n_jl_total, n_py_total,
                            jl_norm, py_norm):
    valid = res.dropna(subset=["r2_identity"])
    r2 = valid["r2_identity"]

    with open(OUTPUT_MD, "w", encoding="utf-8") as f:
        f.write("# Python vs Golden Julia (Canonical): Comparison Report\n\n")
        ...
        if len(poor_df) > 0:
            f.write("| Column | Category | Stat | R2 | Spearman | MAD | Julia NAs | Python NAs | NA Mismatch |\n")
            f.write("|--------|----------|------|----|----------|-----|-----------|------------|-------------|\n")
            for _, row in poor_df.iterrows():
                ...
                f.write(f"| `{row['column']}` | {row['category']} | {row['stat_type']} | "
                        f"{r2v} | {rho} | {mad} | {row['jl_na_count']} | {row['py_na_count']} | {row['na_mismatch']} |\n")
        else:
            f.write("All columns have R2 >= 0.99.\n")
        f.write("\n")

        f.write("---\n")
        f.write("*Generated by `docs/benchmarks/compare_python_vs_golden_julia.py`*\n")
