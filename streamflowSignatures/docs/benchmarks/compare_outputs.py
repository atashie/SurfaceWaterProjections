...
def generate_report(
    ...
    with open(output_path, "w") as f:
        ...
        f.write("---\n")
        f.write("*Generated by validation/compare_outputs.py*\n")
...
def main():
    ...
    if not test_data_dir.exists():
        ...
        print("Please run tests/generate_golden_outputs.R first to create test data.")
        sys.exit(1)

    if not golden_path.exists():
        print(f"\nERROR: Golden outputs not found: {golden_path}")
        print("Please run tests/generate_golden_outputs.R first to generate golden outputs.")
        sys.exit(1)
