...
function calculate_qp_seasonality(
    ...
    # Generate statistics
    sd_stats = generate_stats(annual_data; value_cols=["qp_slope_sd"], trend_completeness=trend_completeness, decade_completeness=decade_completeness)
    merge!(result, sd_stats)

    # Bimodality - filter NaN
    bi_df = annual_data[.!isnan.(annual_data.qp_bimodality), :]
    if nrow(bi_df) >= 3
        bi_stats = generate_stats(bi_df; value_cols=["qp_bimodality"], trend_completeness=trend_completeness, decade_completeness=decade_completeness)
